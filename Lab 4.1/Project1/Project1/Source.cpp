...
public:
    ...
    Complex operator=(Complex& A) {
        ...
        delete[] arr;
        arr = NULL;
        this->arr = new Value * *[A.size[0]];
        for (int i = 0; i < A.size[0]; ++i) {
            this->arr[i] = new Value * [A.size[1]];
        }
        for (int i = 0; i < A.size[0]; ++i) {
            for (int j = 0; j < A.size[1]; ++j) {
                this->arr[i][j] = new Value[A.size[2]];
            }
        }
        ...
        for (int i = 0; i < size[0]; ++i) {
            for (int j = 0; j < size[1]; ++j) {
                for (int k = 0; k < size[2]; ++k) {
                    this->arr[i][j][k].a = A.arr[i][j][k].a;
                    this->arr[i][j][k].i = A.arr[i][j][k].i;
                }
            }
        }
        return Complex(A);
